...
// /proc/stat나 다른 API로 디스크 사용량을 측정할 수 없으므로,
// statvfs() 함수를 사용하여 "/" 파일시스템의 사용량(%)을 계산합니다.
...
// 현재 날짜를 이용하여 텍스트 로그와 CSV 파일에 기록하는 함수
void write_log() {
...
void delete_old_logs() {
    ...
    if (d) {
        while ((dir = readdir(d)) != NULL) {
            // 파일명이 "_log_"와 ".log" 또는 ".csv"를 포함하면 로그 파일로 간주
            if ((strstr(dir->d_name, "_log_") != NULL) &&
                (strstr(dir->d_name, ".log") != NULL || strstr(dir->d_name, ".csv") != NULL)) {
